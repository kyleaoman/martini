...
while True:
    ...
    if "{:s}.{:s}.{:d}".format(*version, rcc) in errtxt.split(
        "from versions: "
    )[1].split(")")[0].split(", "):
        break
...
run_chk(
    "conda create -y --name={:s}-test python={:s}".format(pkgname, pyversion)
)
try:
    CP = subprocess.run(
        ". {:s} && ".format(condash)
...
conf = input(
    "Ready to upload {:s}-{:s}.{:s} to PyPI. Confirm?".format(
        pkgname, *version
    )
)
if conf in ("y", "Y", "yes", "YES", "Yes"):
    twine_upload(
        twine_settings, (os.path.join("dist", "{:s}*".format(distprefix())),)
    )
