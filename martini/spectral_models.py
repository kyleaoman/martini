...
class _BaseSpectrum(object):
    ...
    def init_spectra(self, source, datacube):
        ...
        datacube : martini.DataCube instance
            DataCube object defining the observational parameters, including spectral channels.
        """
        ...
        self.spectra = (A[..., np.newaxis] * self.spectral_function(
            np.tile(channel_edges.value[:-1], vmids.shape + (1,)) * channel_edges.unit,
            np.tile(channel_edges.value[1:], vmids.shape + (1,)) * channel_edges.unit,
            np.tile(vmids.value, np.shape(channel_edges[:-1]) + (1,) * vmids.ndim).T * vmids.unit,
    ...
    def half_width(self, source):
        """
        Abstract method; calculate the half-width of the spectrum, either globally or per-particle.

        Parameters
        ----------
        source : martini.sources.SPHSource (or derived class) instance
            The source object will be provided to allow access to particle properties.
        """
        pass
    ...
    def spectral_function(self, a, b, vmids, **kwargs):
        ...
        Should calculate the flux in each spectral channel, calculation should be vectorized (numpy).
    ...
    def spectral_function_kwargs(self, source):
        ...
        ----------
        source : martini.sources.SPHSource (or derived class) instance
            The source object will be provided so that its attributes can be accessed.
...
class GaussianSpectrum(_BaseSpectrum):
    ...
    def spectral_function_kwargs(self, source):
        """
        Helper function to pass particle velocity dispersions to the spectral_function.
    ...
    def half_width(self, source):
        """
        Calculate velocity dispersions from particle temperatures, or return constant.
